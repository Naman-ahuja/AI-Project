...
#     result.geometry("{0}x{1}+0+0".format(result.winfo_screenwidth(), result.winfo_screenheight()))
    ...
    result.title(f"Predicted Personality : {personality}")
    
#     #Title
#     titleFont = font.Font(family='Arial', size=40, weight='bold')
    Label(result, text=f"Result - Predicted Personality : {personality[0]}", foreground='green', bg='white', font=titleFont, pady=10, anchor=CENTER).pack(fill=BOTH)
